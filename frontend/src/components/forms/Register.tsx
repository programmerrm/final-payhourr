...
export const RegisterForm = () => {
    ...
    return (
        <div className="flex flex-col gap-y-5 py-6 px-2 sm:py-10 sm:px-8 w-full max-w-2xl bg-white rounded-2xl relative mx-auto overflow-y-auto max-h-[90vh]">
            ...
            <form className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 w-full" onSubmit={handleSubmit(onSubmitForm)}>
                ...
                <div className="flex flex-col gap-y-1">
                    <Field label={"Password"} error={errors.password}>
                        <div className="w-full relative">
                            ...
                            >
                                {show.password ? (
                                    <IoMdEyeOff className="text-gray-500 text-lg md:text-xl" />
                                ) : (
                                    <IoMdEye className="text-gray-500 text-lg md:text-xl" />
                                )}
                            </button>
                        </div>
                ...
                <div className="flex flex-col gap-y-1">
                    <Field label={"Confirm Password"} error={errors.confirm_password}>
                        <div className="w-full relative">
                            ...
                            />
                            <button
                                className="absolute top-1/2 -translate-y-1/2 right-2 cursor-pointer z-50"
                                type="button"
                        ...
                        <div className="flex flex-row flex-wrap items-center gap-x-5">
                            <label className="flex items-center gap-x-2.5 cursor-pointer">
                                <input
                                    ...
                                    onChange={() => setValue("role", selectedRole === "buyer" ? "" : "buyer")}
                                />
                                Buyer
                            </label>
                            <label className="flex items-center gap-x-2.5 cursor-pointer">
                                <input
                                    ...
                                    onChange={() => setValue("role", selectedRole === "seller" ? "" : "seller")}
                                />
                                Seller
                            </label>
                        </div>
                ...
                <div className="col-span-full">
                    <div className="flex flex-col gap-y-2">
                        <label className="flex items-start gap-x-2.5 cursor-pointer">
                            <input
                                {...register("terms_accept", { required: "You must accept the terms and conditions." })}
                                type="checkbox"
                                className="w-4 h-4 mt-1"
                            />
                            <span className="text-sm font-medium">
                                I agree to the <Link to="/terms-and-conditions/" className="text-blue-500 hover:underline" onClick={handleCloseForm}>Terms and Conditions</Link> and <Link to="/privacy-policy/" className="text-blue-500 hover:underline" onClick={handleCloseForm}>Privacy Policy</Link>
                            </span>
                        </label>
                    </div>
                </div>
                <div className="col-span-full">
                    <button
                        type="submit"
                        className="text-base font-medium py-3 px-5 text-white bg-blue-600 hover:bg-blue-700 transition-all duration-200 rounded-full w-full"
                    >
                        Register
                    </button>
                </div>
            </form>
            <div className="flex flex-col flex-wrap justify-center items-center w-full mt-2">
                <p className="text-base font-medium ">Already have an account? <button className="text-blue-500" type="button" onClick={handleSwitchToRegister}>Login</button></p>
            </div>
        </div>
    );
}
