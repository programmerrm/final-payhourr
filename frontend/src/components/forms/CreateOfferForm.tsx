...
export const CreateOfferForm: React.FC<CreateOfferFormProps> = ({
    ...
    const onSubmitForm = async (data: FormData) => {
        try {
            ...
            if (data.reference_file && data.reference_file[0]) {
                formData.append("reference_file", data.reference_file[0]);
            }
            const paymentResponse = await addInitPayment(formData).unwrap();
        ...
        <AnimatePresence>
            ...
            >
                <motion.div
                    ...
                    className="bg-white rounded-xl w-full max-w-xl p-6 relative shadow-lg overflow-y-auto max-h-[90vh]"
                >
                    <button
                        onClick={onClose}
