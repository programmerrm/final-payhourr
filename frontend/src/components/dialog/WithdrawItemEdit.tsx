...
import { useAddWithdrawUpdateMutation } from "../../redux/features/payments/paymentsApi";
...
export const WithdrawItemEdit: React.FC<Props> = ({ open, onClose, deposit, refetch }) => {
    const [status, setStatus] = useState(deposit?.status || "");
    const [addWithdrawUpdate, { isLoading }] = useAddWithdrawUpdateMutation();
    ...
    const handleSubmit = async () => {
        try {
            await addWithdrawUpdate({ id: deposit.id, status }).unwrap();
            onClose();
            refetch();
        } catch (error) {
